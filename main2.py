...
def restruction(s_color, d_clolor, mode =0):
    ...
    if mode == 3:
        ...
        Frrrr = (s_color[:, 0]).reshape(num_colors, 1) ** 4
        Fgggg = (s_color[:, 1]).reshape(num_colors, 1) ** 4
        Fbbbb= (s_color[:, 2]).reshape(num_colors, 1) ** 4
        F = np.hstack([ones, s_color, Fbg, Fbr, Fgr, Frr, Fgg, Fbb, Frrr, Frrg, Frrb,
                       Fggg, Fggr, Fggb, Fbbb, Fbbr, Fbbg, Frgb,Frrrr, Fgggg, Fbbbb])  # добавил одну степень полинома
    elif mode == 4:
        # root 1
        Fbg = (s_color[:, 2] * s_color[:, 1]).reshape(num_colors, 1) ** 0.5
        Fbr = (s_color[:, 2] * s_color[:, 0]).reshape(num_colors, 1) ** 0.5
        Fgr = (s_color[:, 1] * s_color[:, 0]).reshape(num_colors, 1) ** 0.5
        ...
        Fbr = (s_color[:, 2] * s_color[:, 0]).reshape(num_colors, 1) ** 0.5
        Fgr = (s_color[:, 1] * s_color[:, 0]).reshape(num_colors, 1) ** 0.5
        Fbbg = (s_color[:, 2] * s_color[:, 2] * s_color[:, 1]).reshape(num_colors, 1) ** (1 / 3)
        Fbbr = (s_color[:, 2] * s_color[:, 2] * s_color[:, 0]).reshape(num_colors, 1) ** (1 / 3)
        Fggb = (s_color[:, 1] * s_color[:, 1] * s_color[:, 2]).reshape(num_colors, 1) ** (1 / 3)
        Fggr = (s_color[:, 1] * s_color[:, 1] * s_color[:, 1]).reshape(num_colors, 1) ** (1 / 3)
        Frrg = (s_color[:, 0] * s_color[:, 0] * s_color[:, 1]).reshape(num_colors, 1) ** (1 / 3)
        Frrb = (s_color[:, 0] * s_color[:, 0] * s_color[:, 2]).reshape(num_colors, 1) ** (1 / 3)
        Frgb = (s_color[:, 0] * s_color[:, 1] * s_color[:, 2]).reshape(num_colors, 1) ** (1 / 3)
